		...
		<div id="content_instrumenty">
			...
			<tbody>
				...
				<?php
					  ...
					  $sql = 'SELECT * FROM produkty WHERE kategoria = "elektronika"';
					  $stmt = $db->query($sql);
							 ...
							 while($row = $stmt->fetch()) 
							{
								...
								echo "<tr>
								<td>
								<a style='color:white' href=produkty_2.php?id=".$row['id_produktu'].">".$row['nazwa_produktu']."</a>
								</td>
								<td>".$row['producent']."</td>
								<td>".$row['cena']."</td>
								<td> 
								<img src=".$row['obrazki']." alt=".$row['nazwa_produktu']." width='220px' height='220px'/>
								</td>
								</tr>";
